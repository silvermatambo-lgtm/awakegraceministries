...
const SERVICES = ['Plan a Visit', 'Prayer Request', 'Children’s Ministry', 'Missions / Charity', 'Giving / Donations', 'General Enquiry'];
const WHATSAPP_NUMBER = '447534030186';
const BUSINESS_NAME = 'Awake Grace Ministries International – UK';
      ...
      {open && (
        <div className="animate-chat-slide mb-3 w-[calc(100vw-2rem)] max-w-[360px] overflow-hidden rounded-3xl border border-stone-200 bg-white shadow-2xl">
          ...
          <div className="min-h-[280px] p-5">
            {step === 'welcome' && <><Bubble>Hi 👋 Welcome to {BUSINESS_NAME}. I can help you plan a visit, request prayer, ask about ministries or send an enquiry.</Bubble><Bubble>I'll collect a few details first, then prepare your WhatsApp message.</Bubble></>}
            ...
            {step === 'message' && <div><p className="mb-3 text-sm font-semibold text-stone-800">Tell us briefly what you need.</p><textarea autoFocus rows={5} value={data.message} onChange={e=>setData({...data,message:e.target.value})} placeholder="Your message, preferred assembly, prayer need or enquiry..." className="w-full resize-none rounded-xl border border-stone-200 p-3 text-sm outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-200"/></div>}
